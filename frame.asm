...
start:
	...
	call skip_spaces
	call atob
	mov color, al      ;color = al = color of symbols in frame


	push bx  ;save bx = address of symbol in command line
	call count_left_high_point   
	pop bx

	call skip_spaces
	call evaluate_address_of_beginning_style   ;si = address on style
...
;--------------------------------------------------------------------------------------------------------------
;											 atob
;translate str to 2 bytes int (bin number)
;
;Entry: bx = address of symbol in command line
;
;Exit:  ax = int number from str
;       bx = address of symbol in command line; symbol != '0-1' (not a digit)
;
;Destr: ax = reading int from str
;		bx = shifting address of symbol in command line before it finds '0-1'
;       cx = read 1 digit and participates in calculations
;		dx = for calculations
;--------------------------------------------------------------------------------------------------------------

atob proc      ;mov ax, 0030d   

	xor ax, ax   ;ax = 0	

	check_next_symbol_in_atob:

	mov cl, ds:[bx]  ;cl = [bx]

	cmp cl, '0'     ;cl - '0' < 0 => end_atob
	js end_atob

	cmp cl, '2'     ;'2' - cl < 0 => end_atob
	jns end_atob
...
;--------------------------------------------------------------------------------------------------------------
;										evaluate_address_of_beginning_style
;find address on the beginning of our style 
;
;Entry: bx = address of symbol in command line
;
;Exit:  si = address on line with our style
;
;Destr: si = for evaluating of our address
;		ax = for calculations: has the number of style --> has shifting for beginning of our style  
;		bx = address shifting (destr in atoi)
;--------------------------------------------------------------------------------------------------------------

evaluate_address_of_beginning_style proc   

	call atoi    ;ax = number of style

	mov si, 0009d
	mul si              ;ax = ax * 9d (every style has 9 symbols => shifting for beginning of our style = number_of_style * 9 symbols)

	mov si, offset frame_style   ;si = address on line with all styles
	add si, ax                   ;si = &frame_style + 9 * number_style
